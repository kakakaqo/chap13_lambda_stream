...
public class Lambda005 {
	...
	public static void main(String[] args) {
		 ...
		 * 스레드의 생성자에 Runnable 인터페이스를 구현한 익명 구현 객체를 바로 정의히
		 * 선언하면서 바로 객체화 됨.
		 */
		...
		// [3] [2]번을 람다식으로 표현
		//     Runnable 구현하는 익명 객체를 람다식을 사용하여 구현(run()메소드를 람다식으로 표현)
		Runnable runnable2 = () -> {
			for (int i = 1; i <= 10; i++) {
				System.out.println("runnable2 : " + i);
			}
		};
		Thread thread2 = new Thread(runnable2); // Thread 생성자에 Runnable 인터페이스 타입 변수
		 ...
		 */
		Thread thread3 = new Thread(() -> { // Runnable 을 구현한 익명 구현 객체 탄생
			for (int i = 1; i <= 10; i++) {
